...
Thanks for your interest in improving @makeitaquote/miqx!
...
| `npm run check:build` | Verify `dist/` after building — see `scripts/check-build.js` |
| `npm run checkupdates` | List available minor/patch dependency updates (`npm-check-updates`) |
| `npm run migrate` | Apply a Biome config migration, when one is needed |

Before opening a pull request, make sure the full set passes:

```sh
npm run ci && npm run typecheck && npm run test && npm run build && npm run check:build
```
